...
接手时发现一个没预告的3天commit空窗——但不是算力空窗：09-20那个会话其实在
自己写完交接文档（12:16）之后又继续干到23:38，只是没再补一份新交接。靠
`git log`加这份日志自己的结尾重建了现状：homo全库campaign已完全收尾（单XGB
0.684，单GNN 0.558），Rec-2统一实验升级到GREEN（naive_merge在5/5个seed里都
赢cross_only，bootstrap置信区间不含0），`merge_homo_sp.py`的pandas-3.0
`astype(str)`对NaN处理的bug已修复（commit `2ea243f`，SP-retry恢复后覆盖率
92.5%），还有一个750对cross-explore批次（`26961707`）在23:23提交了，但会话
结束时还没跑完。按照那条还没解决的"已跟踪文件离奇消失"的线索先查了一遍
`git status --short | grep '^ D'`——这次是干净的，0个被删文件。

**cross-explore 750对批次：全部跑完了（94/94个shard），100%成功打标，0失败。**
用`merge_rec1_b973c_tierB.py`指向新的shard目录做了合并（脚本对shard schema是
通用的，不用改代码）——**覆盖率750/750（100%）**，全部可用，没有真失败也没有
带错误标记的行。残差散布QC（沿用脚本原本为自洽几何设计的口径）：`resid_gxtb`
标准差2.413，`resid_b973c`标准差0.87，**标准差比值0.361**——**GREEN**，跟最初
D-pilot的0.26很接近，明显落在<=0.45的阈值以内。这次是B97-3c基线杠杆在一批
全新的、从未打过标的cross pair上依然成立（不是重新检查已有标签的数据），
比之前那些回顾性检查的验证力度更强。这750对是新的、真实DFT打标的cross数据——
还没并入任何训练表；要不要并入（重训节奏、750对在当前22,529+的cross训练集
规模下能不能带来实质提升）是个真决定，值得跟用户报一下而不是直接自己做，
所以先在`CAMPAIGN_PLAN.md`/`PROJECT_PLAN.md`里记成"可用库存"，不擅自开新
一轮重训。

**homo aldgap-regen job（`26957684`，1,131个任务，`%40`节流，09-20 20:13
提交）还在跑**：截至本条目204/1131已完成，40个RUNNING，其余排在节流后面
排队。按目前观察到的~2-2.5 CPU小时/对的速度，这是个多天量级的campaign
（粗估还要~2-3天才能跑完）——不需要干预，等跑完或接近跑完时用修好的
`merge_homo_sp.py`再合并一次。这次条目没有重跑中间merge，因为上一次
（93.73%覆盖率，只算了SP-retry那部分）依然是最新状态，aldgap这条线在
204/6,784对等量任务这个进度上还没实质性推进geom_extract_fail的数字。
