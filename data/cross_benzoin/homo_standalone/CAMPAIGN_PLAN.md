...
| **single attentive GNN** (`gnn_only_mae`, the number to quote) | **0.558** | -- |
| (informational only, not reported) tabular-ensemble/GNN blend, w=0.60 | 0.543 | -- |
| raw baseline, no model | 5.029 | 0.900 |

Both reported numbers land far ahead of the pre-purge full-library homo model
(~MAE 10) and close to the cross champion (0.528) -- likely because a homo
row (donor == acceptor) is a structurally simpler learning problem than a
cross row, on top of the same b973c-baseline lever. The GNN (0.558) actually
beats the single XGB (0.684) here, unlike most of this project's cross-side
history where XGB/blend usually leads -- worth keeping in mind if this
pattern recurs elsewhere. See LAB_JOURNAL.md(+ZH) 09-20 for the full writeup,
including a note to revisit PROJECT_PLAN.md's "homo is not harder than cross"
