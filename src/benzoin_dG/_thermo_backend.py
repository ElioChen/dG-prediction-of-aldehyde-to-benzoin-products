...
_BENZOIN_RXN = AllChem.ReactionFromSmarts(
    "[CX3H1:1](=[O:2]).[CX3H1:3](=[O:4])>>[C:1]([OH1:2])[C:3](=[O:4])"
)
...
def _auto_nconfs(rotbonds: int, n_confs_max: int, n_heavy: int = 0) -> int:
    if rotbonds <= 3:
        n = 1
    else:
        frac = min(1.0, (rotbonds - 3) / 6.0)
        n = min(n_confs_max, max(3, round(n_confs_max * frac)))
    # Heavy-atom cap: each conformer is xTB-optimised (--opt tight) and cost scales
    # steeply with size, so bound the count for large benzoin products to keep a
    # single featurization from grinding for hours (mirrors the labeling-path cap in
    # pipeline/compute/conf_funnel_v2._frust_nconfs_v2).
    if n_heavy >= 70:
        n = min(n, 2)
    elif n_heavy >= 55:
        n = min(n, 3)
...
def _build_best_xyz(
    ...
    n  = _auto_nconfs(rb, n_confs_max, _mol_heavy_atoms(smiles))
...
def _build_orca_input(
    ...
    if method.lower().endswith("-3c"):
        kw_parts = [method, "TightSCF", "NoMOPrint"]
    else:
        func_kw = _build_orca_keywords(method)
        kw_parts = [func_kw, basis, "RIJCOSX", "def2/J", "TightSCF", "NoMOPrint"]
    if solv_kw:
...
def _mol_pipeline(
    ...
    *,
    ...
    res: _MolResult = {"G": None, "E_el": None, "G_sh": None,
                       "E_orca": None, "G_orca": None, "errors": []}
    ...
    if ensemble_k > 1:
        ...
        ranked = _rank_conformers(smiles, work_dir / "rank", xtb_bin,
                                  n_confs_max, label, solvent=solvent)
        if not ranked:
    ...
    best_xyz, _ = _build_best_xyz(smiles, work_dir, xtb_bin, n_confs_max,
