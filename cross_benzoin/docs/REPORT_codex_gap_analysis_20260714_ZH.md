...
**截至撰写本报告时:SLURM job 24607515(array 0–5,每个任务一个 100 对
的 chunk,genoa 分区,6 个节点)正在运行,进度很快**——运行约 9 分钟后
已完成 600 行中的 88 行(6×12=72 个并发 worker,整体约每分钟 10 行),
按此速度预计约一小时左右可全部跑完,远在 12 小时的任务时限之内。目前为止
**全部零错误**——这是本项目历史上第一批真实的 cross-benzoin ΔG 数值
(此前项目里 donor≠acceptor 的 cross 行 ΔG 全部是零,之前所有计算都只覆盖
homo/对角线情形):

- `dG_xtb_kcal`(n=57 快照):均值 −11.4,标准差 2.9,范围 [−18.2, −1.9] kcal/mol。
- `dG_gxtb_kcal`(n=57 快照):均值 +2.1,标准差 3.0,范围 [−3.1, +10.4] kcal/mol。
- AB/BA 方向性合理性检查通过:交换 donor/acceptor 会显著改变
  `dG_xtb_kcal`(例如某一对:一个方向 −5.96,另一个方向 −11.35)——证实
  两条定向记录确实是不同的区域异构体,而不是元数据互换的重复项,与
  codex 此前 250/250 AB≠BA 产物枚举检查的结论一致。
...
数组跑完后,本报告会补充完整的 `dG_xtb_kcal`/`dG_gxtb_kcal` 分布以及按
类别组合的细分结果。可用以下命令查看当前进度:
