#!/usr/bin/env python
"""B97-3c-baseline variant of blend_gnn_seed_ensemble_r10.py, per
rec1_b973c_tierB/DRAIN_RUNBOOK.md step 5. Averages the 4 b973c-Delta GNN
seeds (gnn_attentive_10rounds_b973c_seed{1..4}, no separate "v1" champion
seed at this baseline -- the b973c campaign only trained seeds 1-4) and
re-sweeps the blend weight against the b973c champion XGB ensemble
(scaffold_disjoint_10rounds_b973c_v1).

  CB_TARGET_COL=dG_r2scan_kcal CB_BASELINE_COL=dG_b973c_kcal \
    /home/schen3/venv/nequip/bin/python cross_benzoin/blend_gnn_seed_ensemble_r10_b973c.py
"""
...
from train_cross_delta import TARGET_COL, BASELINE_COL  # noqa: E402 -- env-overridable, see DRAIN_RUNBOOK
...
ENS_PATH = NEW / "models/ensemble_scaffold_disjoint.joblib"
SEED_DIRS = [R10 / f"gnn_attentive_10rounds_b973c_seed{s}" for s in (1, 2, 3, 4)]
SHIPPED_MAE = 2.215  # old champion, for the delta column only -- not this campaign's baseline
...
def main() -> int:
    print(f"TARGET_COL={TARGET_COL} BASELINE_COL={BASELINE_COL}")
    ...
    for d in have:
        ...
        true_by_id = tp
        print(f"  {d.name:36s} GNN-only holdout MAE {mae:.4f}")
    ...
    print(f"\n{'#seeds':>7} {'gnn_ens_MAE':>12} {'best_w_gnn':>11} {'blend_MAE':>10} {'vs shipped 2.215':>16}")
    rows = []
    for k in range(1, len(per_seed_pred) + 1):
        ...
        print(f"{k:>7} {mae_gnn:>12.4f} {best_w:>11.2f} {best_mae:>10.4f} {best_mae - SHIPPED_MAE:>+16.4f}")
        rows.append({"n_seeds": k, "gnn_ens_only_mae": mae_gnn, "best_w_gnn": best_w,
                     "blend_mae": best_mae, "delta_vs_shipped_2215": best_mae - SHIPPED_MAE})

    out = R10 / "gnn_seed_ensemble_r10_b973c_result.json"
