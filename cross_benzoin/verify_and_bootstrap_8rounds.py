#!/usr/bin/env python
"""
Verify the repackaged CrossBenzoinBlendPredictor (round1-8 champion/ensemble +
attentive-pooling GNN) reproduces the training-time MAE, then bootstrap the
blend-vs-ensemble-only delta for significance. Mirrors verify_and_bootstrap_721.py's
method, applied to the round1-8 + attentive-architecture candidate champion.
"""
import sys
import json
...
df = pd.read_parquet(
    "data/cross_benzoin/cross_round8/cross_train_table_8rounds_scaffold_split_labeled_slim260.parquet")
test = df[df["new_scaffold_split"] == "test"].reset_index(drop=True)
print("test n =", len(test))

pred = CrossBenzoinBlendPredictor.load(
    "data/cross_benzoin/cross_round8/scaffold_disjoint_8rounds_v1",
    gnn_dir="data/cross_benzoin/cross_round8/gnn_attentive_8rounds_v1",
)
dG_pred = pred.predict(test)
actual = test["dG_orca_kcal"].to_numpy()
err_blend = np.abs(dG_pred - actual)
mae_blend = err_blend.mean()
print(f"packaged predictor MAE on 8rounds test (n={len(test)}): {mae_blend:.4f} "
      f"(reported best_blend_mae=2.106)")
...
print(f"ensemble-only MAE: {mae_ens:.4f}  (reported 2.201)")
...
outp = "data/cross_benzoin/cross_round8/gnn_attentive_8rounds_v1/bootstrap_significance.json"
