#!/usr/bin/env python3
"""
Aldehyde Descriptor Calculator — QM-only (no Level 0)
=====================================================
Derived from ald_descriptors.py with the entire Level 0 (RDKit 2D descriptor)
block REMOVED. The cheminformatics 2D descriptors (MW/LogP/TPSA/ring counts/
BertzCT/Kappa2/… and n_CHO) are size-correlated, geometry-free, and live in the
library CSV already; the geometry/QM pipeline below is the value-add. The pure-
SMILES 2D surrogate still gets them from ald_descriptors.calc_rdkit, which this
file leaves untouched.

Pipeline per aldehyde SMILES (RotBonds is still computed inline, solely to scale
the conformer count for flexible molecules — it is NOT written to the output):
  ...
  Level 2  (morfeus-ml)   : %Vbur, Sterimol L/B1/B5, SASA, Dispersion P_int
  Level 3  (Multiwfn)     : ADCH charges + Fukui, QTAIM C=O BCP
...
Usage
-----
  python ald_descriptors_qm.py                        # reads aldehydes.csv
  python ald_descriptors_qm.py --max 100              # first 100 rows
  python ald_descriptors_qm.py --smiles "O=Cc1ccccc1"
  python ald_descriptors_qm.py --xtb-bin /path/to/xtb
  python ald_descriptors_qm.py --multiwfn-bin /path/to/Multiwfn --multiwfn
  python ald_descriptors_qm.py --no-xtb-opt           # skip opt, use MMFF only
...
_XTB_FIELDS = [
    ...
    "wbo_CO",                       # xtb_alpha0/xtb_C6 dropped: alpha0≈MolMR r=0.998, both≈MW (size)
    "pa_CHO_O",
]
...
_MWF_FIELDS = [
    # NB: GFN2 wfn HOMO/LUMO/gap dropped in the v4 slim-down — they were r=1.000
    # identical to the Level-1 xtb_HOMO/LUMO/gap (same GFN2 orbitals, parsed twice).
    ...
    # QTAIM C=O bond critical point (π character + bond polarity). qtaim_rho_CO dropped:
    # r=0.913 with the free xTB wbo_CO; Laplacian/ellipticity carry the non-redundant info.
...
def _parse_qtaim_bcp_from_file(
    ...
    """
    result: dict[str, Any] = {"qtaim_lap_CO": None, "qtaim_ell_CO": None}
...
def calc_multiwfn(
    ...
    # (wfn HOMO/LUMO/gap dropped — identical to Level-1 xtb_*; the molden is still
    #  generated above for the ADCH charges + QTAIM analysis below.)
    ald_atoms = find_aldehyde_atoms(symbols, coords)
    if not ald_atoms:
...
def _stem(name: str, idx: int) -> str:
    safe = re.sub(r"[^\w-]", "_", name)[:50]   # pulled out of the f-string for py<3.12 compat
...
def process_one(
    ...
    # RotBonds is the only RDKit-2D quantity still needed (Level 0 removed); compute it
    # inline solely to scale the conformer count — it is not part of the output.
    use_nconfs = 1
    if n_confs > 1 and xtb_bin and do_xtb_opt:
        _m2d = Chem.MolFromSmiles(smiles)
        rotbonds = rdMolDescriptors.CalcNumRotatableBonds(_m2d) if _m2d else 0
        use_nconfs = _auto_nconfs(rotbonds, n_confs)
...
_ALL_FIELDS = (
    ...
    _XTB_FIELDS + _MORFEUS_FIELDS + _MWF_FIELDS
)
