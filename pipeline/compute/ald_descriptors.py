...
_RDKIT_CALC = [
    ...
    # MolMR/Chi0v/Chi1v/Kappa1/LabuteASA dropped in slim-down: all r>0.94 with MW (size duplicates).
]

_RDKIT_FIELDS = [n for n, _ in _RDKIT_CALC] + ["n_CHO"]
...
def calc_rdkit(smiles: str) -> dict[str, Any]:
    ...
    # Aldehyde count only. Gasteiger partial charges were dropped in the v4 slim-down:
    # crude empirical 2D charges, no Se parameterization, and superseded by the two QM
    # charge schemes computed downstream — xTB Mulliken (free) and Multiwfn ADCH.
    try:
        desc["n_CHO"] = len(mol.GetSubstructMatches(Chem.MolFromSmarts("[CX3H1](=[O])")))
    except Exception:
        desc["n_CHO"] = None
