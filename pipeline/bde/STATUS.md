...
|---|---|---|---|---|---|---|---|
| marked (CRG 标记目标键) seed0 | `26807944` | COMPLETED,3:48:14 | 138268/15363/17365 | 3.1404 | 5.338 | 0.8859 | 0.9453 |
| unmarked (`--no-mark` 对照) seed0 | `26807945`→重提交 `26829622`(08:00:00 墙钟) | COMPLETED,4:13:05 | 138268/15363/17365 | 3.1377 | 5.314 | 0.8869 | 0.9454 |

两组 MAE/R² 都落在判断标准区间内(2-4 kcal/mol, R² 0.85-0.9,对照 B6 单 seed
2.09/3.19),**确认 checkpoint-restore 修复生效**。**seed0 单点结论:marked 与
unmarked 几乎完全相同**(MAE 差 0.0027 kcal/mol,noise量级)——CRG 显式标记目标键
对 BDE 产物侧预测**没有看出可衡量的收益**,跟 dG 侧"标记确实带来可衡量差异(虽然不如
调优后的champion)"的情形不一样。可能的原因(文件自己docstring里提过的假说):
每一行本来就只问"这一行自己的 ketC-carbC"/"这一行自己的formyl C-H",分子类别的
同质性已经隐含告诉了模型是哪根键,显式标记是冗余信息。
