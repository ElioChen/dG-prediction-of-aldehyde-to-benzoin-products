#!/usr/bin/env python3
"""
Benzoin aldehyde filter v5 — v4 + drop all vinyl/α,β-conjugated aldehydes.

Identical to filter_smiles_v4.py in every respect EXCEPT it now REJECTS the
`vinyl_conj` CHO-environment class outright (acrolein-like: CHO on a non-aromatic
sp2 carbon). v4 already removed the strict ENAL/YNAL forms, but the SMARTS-based
`enal` rule [CX3H1](=O)[C;!a]=[C;!a] only catches an explicit non-aromatic C=C; it
let through ~910 substrates whose α-carbon is sp2 by other conjugation (cross-
conjugation, C=N, ring sp2, etc.). Those still divert NHC catalysis away from the
classic benzoin pathway, so v5 excludes the whole vinyl_conj class using
`cho_category.cho_class` — the single source of truth — guaranteeing the kept set is
purely {aromatic_carbo, aromatic_hetero, aliphatic}.

v4 and its outputs are left untouched.
...
Rejection taxonomy (first failing rule wins)
  ...
  vinyl_conj           cho_class == vinyl_conj  (any remaining CHO on a non-aromatic
                       sp2 carbon; NEW in v5 — the strict enal/ynal SMARTS miss these)
...
Kept rows get two extra columns
  cho_class  aromatic_carbo / aromatic_hetero / aliphatic
             (single source of truth = cho_category.cho_class; vinyl_conj can no
             longer appear — it is rejected upstream)
...
Usage
  python pipeline/filter_smiles_v5.py
  python pipeline/filter_smiles_v5.py --input /scratch-shared/schen3/aldehydes.csv \
...
def classify(smiles, mw, max_mw: float, max_aliph_c: int) -> str:
    ...
    # NEW in v5: drop any remaining vinyl/α,β-conjugated aldehyde that the strict
    # enal/ynal SMARTS missed (cho_class is the single source of truth).
...
def main() -> int:
    ...
    ap.add_argument("--output", default=str(lib / "aldehydes_clean_v5.csv"))
    ap.add_argument("--rejects", default=str(lib / "aldehydes_rejected_v5.csv"))
    ...
    order = ["keep", "multi_component", "net_charged", "mw_too_high",
             ...
             "alpha_dicarbonyl", "reactive_group", "vinyl_conj",
