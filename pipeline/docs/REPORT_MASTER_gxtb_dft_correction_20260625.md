...
## 7b. CAN BETTER LABELS BREAK THE ~1.6 FLOOR? (2026-06-26) — Boltzmann DFT + wB97X-3c + Optuna
Full analysis: `viz_gxtb_20260625/REPORT_better_labels_20260626.md`. On held-out **test** molecules:
- **Conformer noise is ~0.9 kcal/mol and *removable*.** Re-deriving ΔE=E_prod−2·E_ald with xTB-Boltzmann
  averaging over K=5 conformers (both species) vs the single lowest-xTB conformer: **boltz_corr std 0.92**,
  mean|·| 0.62, corr with model residual −0.07 (pure noise). Retraining on Boltzmann labels could lower the
  floor **~1.57→~1.38** (quadrature) — the single biggest remaining lever, but ~5× DFT cost for ~0.2 kcal.
  (NB: confnoise job 24225782 reports 2.7 kcal *product-only random-conformer std* — an upper bound on a
  different quantity; the honest *protocol* noise is the 0.92 from lowest-vs-Boltzmann on full ΔE.)
- **Functional choice moves labels MORE and systematically.** wB97X-3c − r2SCAN-3c: mean −0.49, std 1.69,
  mean|·| 1.41, and corr **+0.31** with the model residual — part of the apparent model error is r2SCAN-3c
  functional error. But this is a *target-definition* question (no CCSD(T) anchor here), not a free win.
- **Model side exhausted**: Optuna-tuned regularized XGB (job 24225459) test **1.557** (default 1.59,
  ensemble 1.566); tuning ≈ noise.
- **Dominant caveat**: only 95/120 (wB97X-3c 71/95) finished; survivors |g-xTB Δ| 3.73 vs failed 7.96
  (subset MAE 0.97 ≪ global 1.57). The hard EWG/large tail that *sets* the floor is under-sampled — so
  noise estimates describe the easy 79%. **If accuracy <1.4 is ever required: multi-conformer re-label the
  hard tail specifically, not the bulk.**
