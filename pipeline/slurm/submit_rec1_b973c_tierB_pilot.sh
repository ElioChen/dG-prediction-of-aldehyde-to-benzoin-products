#!/bin/bash
#SBATCH --job-name=rec1_tierB_pilot
...
#SBATCH --time=4:00:00
#SBATCH --array=0-1
#SBATCH --output=/gpfs/scratch1/shared/schen3/benzoin-dg-restored/data/cross_benzoin/rec1_b973c_tierB/logs/pilot_%A_%a.out
set -o pipefail
...
OUTD="$REPO/data/cross_benzoin/rec1_b973c_tierB/pilot_shards"
CHUNK=2
...
mkdir -p "$SCR"
OUT="$OUTD/pilot_$(printf '%05d' "$ID").csv"
...
echo "pilot task $ID done rc=$RC rows=$(( $(wc -l < "$OUT") - 1 )) $(date)"
exit $RC
