#!/usr/bin/env python
"""Quick go/no-go: does BDE (aldehyde C(=O)-H + product new-C-C, real xtb homolysis) add
signal on top of the MORDREDSLIM271 champion (72 QM + 199 SHAP-pruned mordred, test MAE
1.525)? Mechanistically motivated (see bde-descriptor-idea memory): the product C-C BDE is
literally the bond whose formation energy the model predicts.

Same 70:20:10 split (seed 42), quick 2-member XGB comparison first (matches the
finalize_correction_mordred.py precedent) -- promote to full MLP+XGB+diagnostics+bundle
treatment only if this shows a real out-of-noise-band gain (>~0.02-0.03 MAE, per the
noise-band study).

Known gap: 2/1098 product BDE chunks (120, 481) hit a deterministic xtb/rdDetermineBonds
hang even at 4h and were not recovered (~400/219k molecules, ~0.18%) -- proceeding with the
99.8% that succeeded, per this project's established "small gap, don't block" pattern.
"""
import json, time
...
def main():
    ...
    # mordredslim271's kept mordred set
    ...
    ald_bde = pd.read_csv(f"{H}/aldehydes_bde_descriptors.csv", usecols=["id", "bde_ald_CH_kcal"]).rename(columns={"id": "ald_id"})
    ...
    print(f"BDE coverage: prod {df['bde_prod_CC_kcal'].notna().mean()*100:.1f}%  "
         f"ald {df['bde_ald_CH_kcal'].notna().mean()*100:.1f}%", flush=True)

    feats_slim271 = FEATS_72 + prod_mrd_cols + ald_mrd_cols
    feats_plus_bde = feats_slim271 + ["bde_prod_CC_kcal", "bde_ald_CH_kcal"]
    print(f"mordredslim271: {len(feats_slim271)} feats; +BDE: {len(feats_plus_bde)}", flush=True)

    results = {}
    for label, feats in [("mordredslim271", feats_slim271), ("mordredslim271_plus_bde", feats_plus_bde)]:
        results[label] = run(df, feats, label)

    rep = OUT / f"REPORT_bde_augment_{TAG}.md"
    with open(rep, "w") as fh:
        fh.write(f"# BDE-augmented feature comparison ({TAG})\n\n")
        fh.write("Same 70:20:10 split (seed 42), XGB_d8+XGB_d10 ensemble (quick check, no MLP), "
                 "vs mordredslim271 preferred production model (test MAE 1.525, full ensemble).\n\n")
        for label, r in results.items():
            fh.write(f"- **{label}**: n_feat={r['n_feat']} n={r['n']:,} "
                     f"MAE={r['mae']:.3f} RMSE={r['rmse']:.3f} R2={r['r2']:.3f} scope={r['scope']}\n")
    json.dump(results, open(OUT / f"bde_augment_results_{TAG}.json", "w"), indent=2)
