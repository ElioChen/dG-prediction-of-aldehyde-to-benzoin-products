"""Extra tabular variants on full 219k (72 feat) -> MLflow exp5: Ridge/Lasso, MLP depths, XGB depths."""
...
mlflow.set_tracking_uri(f"sqlite:///{R}/mlflow_benchmark.db");mlflow.set_experiment("exp5_tabular_variants")
...
dtr,dva=df.delta.values[tr],df.delta.values[va];g0=df.dG_gxtb_kcal.values[te];y=df.dG_orca_kcal.values[te]
print("n",len(df),flush=True)
def log(name,pred):
    yh=g0+pred;mae=float(np.abs(yh-y).mean());r2=float(1-((yh-y)**2).sum()/((y-y.mean())**2).sum())
    with mlflow.start_run(run_name=name):mlflow.log_params({"model":name,"nfeat":len(F),"n":len(df)});mlflow.log_metrics({"test_mae":mae,"test_r2":r2})
    print(f"  {name}: {mae:.3f} R2={r2:.3f}",flush=True)
log("LinearRegression",LinearRegression().fit(Xtr,dtr).predict(Xte))
log("Ridge_a1",Ridge(1.0).fit(Xtr,dtr).predict(Xte))
log("Lasso_a01",Lasso(0.01).fit(Xtr,dtr).predict(Xte))
for hl in [(128,),(256,128),(512,256,128),(512,256,128,64)]:
    log(f"MLP_{'-'.join(map(str,hl))}",MLPRegressor(hidden_layer_sizes=hl,alpha=1e-4,max_iter=250,early_stopping=True,n_iter_no_change=12).fit(Xtr,dtr).predict(Xte))
for d,ne in [(4,800),(6,1200),(8,1500),(10,2000)]:
    ...
    log(f"XGB_d{d}_n{ne}",m.predict(Xte))
