#!/usr/bin/env python
"""Full go/no-go: does BDFE (free-energy-corrected, DMSO-consistent v2 -- aldehyde C(=O)-H +
product new-C-C) add signal on top of MORDREDSLIM271, using BOTH sides now that
products_bdfe2_descriptors.csv exists? Mirrors finalize_correction_bde.py's pattern exactly,
substituting bdfe_xtb_kcal for bde_*_kcal. The aldehyde-only interim check (job 24427785)
showed only a marginal +0.003 MAE gain (noise-level) -- product-side new-C-C BDFE is
mechanistically closer to the reaction coordinate (the bond literally being formed), so this
full test is the real verdict on whether BDFE is worth keeping as a descriptor family.

Same 70:20:10 split (seed 42), quick 2-member XGB comparison (matches finalize_correction_
bde.py / finalize_correction_bdfe_aldonly.py precedent) -- promote to full MLP+XGB+
diagnostics+bundle treatment only if this shows a real out-of-noise-band gain (>~0.02-0.03
MAE, per the noise-band study).
"""
import json, time
...
def main():
    ...
    # BDFE v2 sidecars (DMSO-consistent), both sides
    prod_bdfe = pd.read_csv(f"{H}/products_bdfe2_descriptors.csv", usecols=["id", "bdfe_xtb_kcal"]).rename(
        columns={"bdfe_xtb_kcal": "prod_bdfe_kcal"})
    ...
    # sanity filter: drop pathological non-converged-SCF rows (|BDFE|>200 kcal/mol impossible for a real bond)
    prod_bdfe = prod_bdfe[prod_bdfe["prod_bdfe_kcal"].abs() <= 200]
    ald_bdfe = ald_bdfe[ald_bdfe["ald_bdfe_kcal"].abs() <= 200]
    ...
    print(f"BDFE coverage: prod {df['prod_bdfe_kcal'].notna().mean()*100:.1f}%  "
         f"ald {df['ald_bdfe_kcal'].notna().mean()*100:.1f}%", flush=True)

    feats_slim271 = FEATS_72 + prod_mrd_cols + ald_mrd_cols
    feats_plus_bdfe = feats_slim271 + ["prod_bdfe_kcal", "ald_bdfe_kcal"]
    print(f"mordredslim271: {len(feats_slim271)} feats; +BDFE(both sides): {len(feats_plus_bdfe)}", flush=True)

    results = {}
    for label, feats in [("mordredslim271", feats_slim271), ("mordredslim271_plus_bdfe_both", feats_plus_bdfe)]:
        results[label] = run(df, feats, label)

    rep = OUT / f"REPORT_bdfe_full_augment_{TAG}.md"
    with open(rep, "w") as fh:
        fh.write(f"# Full (aldehyde+product) BDFE-augmented feature comparison ({TAG})\n\n")
        fh.write("Both-sides BDFE (v2, DMSO-consistent) test, now that products_bdfe2_descriptors.csv "
                 "is complete (1435/1463 chunks, 28 timeout, ~1.9% gap accepted). Same 70:20:10 split "
                 "(seed 42), XGB_d8+XGB_d10 ensemble (quick check, no MLP), vs mordredslim271 "
                 "preferred production model (test MAE 1.525, full ensemble) and the interim "
                 "aldehyde-only result (MAE 1.609, delta 0.003 -- noise-level).\n\n")
        for label, r in results.items():
            fh.write(f"- **{label}**: n_feat={r['n_feat']} n={r['n']:,} "
                     f"MAE={r['mae']:.3f} RMSE={r['rmse']:.3f} R2={r['r2']:.3f} scope={r['scope']}\n")
    json.dump(results, open(OUT / f"bdfe_full_augment_results_{TAG}.json", "w"), indent=2)
