#!/usr/bin/env python
"""SLIM variant of finalize_correction_mordred_final.py: 72 champion feats + only the
SHAP-importance + correlation-pruned 199/438 mordred feats (job 24405069,
REPORT_mordred_slim_20260703.md: kept in descending mean|SHAP| order, dropping anything
|corr|>0.9 with an already-kept feature). Tests whether the full MORDRED510 champion
(test MAE 1.517) holds at ~271 feats instead of 510 -- if yes, this becomes the preferred
production model (same accuracy, smaller/more interpretable/faster to score).

Same full production treatment as finalize_correction_mordred_final.py: bundle + full-
library predictions + complete diagnostics.
"""
import json, time
...
def main():
    ...
    # restrict to the SHAP importance + correlation-pruned 199/438 kept set (job 24405069,
    # REPORT_mordred_slim_20260703.md) -- this is the slim variant, not the full 510-feat one
    kept_mordred = set(json.load(open(f"{H}/viz_gxtb_20260625/mordred_slim_selection_20260703.json"))["kept_mordred"])
    prod_mrd_cols = [c for c in prod_mrd_cols if c in kept_mordred]
    ald_mrd_cols = [c for c in ald_mrd_cols if c in kept_mordred]
    ...
    conf = unc < thr
    print(f"mordredslim271 test MAE={mae:.3f} RMSE={rmse:.3f} R2={r2:.3f}", flush=True)
    print(f"confident {conf.mean()*100:.0f}% MAE={err[conf].mean():.3f}  routed MAE={err[~conf].mean():.3f}", flush=True)

    # ── save production bundle ───────────────────────────────────────────
    bundle = MODELDIR / f"gxtb_dft_correction_MORDREDSLIM271_{TAG}.joblib"
    ...
    fl[["id", "smiles", "dG_gxtb_kcal", "dG_gxtb_corrected_final", "uncertainty_pi_width", "route_to_dft"]].to_csv(
        OUT / f"products_dG_corrected_MORDREDSLIM271_{TAG}.csv", index=False)
    ...
    ax.set_title(f"mordredslim271 parity (test, n={len(yte):,})\nMAE={mae:.3f} RMSE={rmse:.3f} R2={r2:.3f}")
    savefig("90_parity_mordredslim271.png")
    ...
    ax.set_title(f"mordredslim271 residual distribution\nmean={resid.mean():.3f} std={resid.std():.3f}")
    savefig("91_residual_hist_mordredslim271.png")
    ...
    ax.set_title("mordredslim271 residual vs predicted")
    savefig("92_residual_vs_pred_mordredslim271.png")
    ...
    ax.set_title(f"mordredslim271 MLP training curve (stopped at epoch {len(mlp.loss_curve_)})")
    savefig("93_mlp_loss_curve_mordredslim271.png")

    for m, name, fname in [(xgb8, "XGB_d8", "94_xgb_d8_curve_mordredslim271.png"),
                           (xgb10, "XGB_d10", "95_xgb_d10_curve_mordredslim271.png")]:
        ev = m.evals_result()
        fig, ax = plt.subplots(figsize=(7, 5))
        ...
        ax.set_title(f"mordredslim271 {name} training curve"); ax.legend()
        savefig(fname)
    ...
    ax.set_title(f"mordredslim271 uncertainty-routing ROC\n(worst {ROUTE_FRAC*100:.0f}% by true error)")
    ax.legend(loc="lower right")
    savefig("96_uncertainty_roc_mordredslim271.png")
    ...
    ax.set_title(f"mordredslim271 error vs uncertainty (conf MAE={err[conf].mean():.3f}, routed MAE={err[~conf].mean():.3f})")
    ax.legend()
    savefig("97_error_vs_uncertainty_mordredslim271.png")
    ...
    ax.set_title("mordredslim271 error vs molecular size"); ax.legend()
    savefig("98_error_vs_molwt_mordredslim271.png")
    ...
    te_df[keep].to_csv(OUT / f"test_predictions_mordredslim271_{TAG}.csv", index=False)

    worst = te_df.sort_values("error", ascending=False).head(50).reset_index(drop=True)
    worst[keep].to_csv(OUT / f"worst_mispredictions_mordredslim271_{TAG}.csv", index=False)
    top20 = worst.head(20)
    mols, legends = [], []
    ...
    if mols:
        ...
        img.save(OUT / "99_worst_mispredicted_aldehydes_mordredslim271.png")
        print("wrote 99_worst_mispredicted_aldehydes_mordredslim271.png", flush=True)

    rep = OUT / f"REPORT_MORDREDSLIM271_FINAL_{TAG}.md"
    with open(rep, "w") as fh:
        fh.write(f"# mordredslim271 production model ({TAG})\n\n")
        ...
        fh.write(f"- Model: `{bundle}`\n- Full-library output: `products_dG_corrected_MORDREDSLIM271_{TAG}.csv` "
