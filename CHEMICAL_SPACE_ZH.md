...
## 5b. 消融实验：只用惰性子集够不够？（2026-09-11）

用户问完 §5 那个修正之后追问"为什么还在算260个、对象是谁、以后可能要重新设计
描述符"——直接测了一下 flying dataset 能不能做成完全惰性（不碰 DFT 也能筛）。
跟生产冠军一样的单-XGB 配方（`cross_benzoin/ablation_lazy_vs_full_features.py`，
round10 表，5 个种子）：

| 特征集 | 特征数 | holdout MAE | R² |
|---|---|---|---|
| FULL（生产冠军） | 257 | **2.544 ± 0.020** | 0.69 |
| LAZY（去掉91个产物QM+product_mordred_列） | 166 | **3.548 ± 0.016** | ~0.38 |
| g-xTB 基线，无模型 | -- | 5.037 | -0.14 |

**结论：不够。** 去掉的这91个（产物侧QM+Mordred，约占schema的1/3）带着真实
信号——MAE 掉了 +1.00 kcal/mol（+39.5%），种子间标准差只有 ~0.02，是噪声的
~50倍，不是巧合。惰性子集比原始基线好很多（可以当一个不用DFT的初筛器），但
替代不了完整流水线。**对未来描述符重设计的启示**：不要想着砍掉产物侧的几何
信息，要想**更便宜地拿到它**（比如快速几何代理、粗略/部分计算、或者一个不用
真优化就能从两个醛推断出产物侧信号的学到的编码器）——"重新设计描述符"从一个
纯特征选择问题变成了一个计算成本问题。
...
### 第6步，零新算力试点（2026-09-20）

用户："先不启动"新的 AL/DFT 战役——先把第6步的基础设施打通，看看真实空间
长什么样。

- 实现了 `FlyingDataset.sample(n, seed, exclude_known, only_computable)`
  （之前只在 §7 文档里提过，代码没写；`iter_pairs()` 的无限制模式仍然会
  故意抛 `NotImplementedError`，没变）。均匀随机抽地址，零 DFT/xTB 开销。
- `cross_benzoin/train_lazy_absolute_screener.py`：只用 `pair()` 对一个
...
### 用真实DFT验证试点的方向性结论（2026-09-20/21）

`cross_explore_20260920` 批次（同样用`sample()`均匀抽样，750对，真正
自洽的新DFT标签，0个错误）给了真实的ground truth来检验零成本筛选模型
23.1%这个结论：**750个真实标记的pair里，32.4%是favorable（dG<0）**——
比零成本模型的23.1%更接近历史AL标记集合的35.7%。**筛选模型"真实空间比
AL标记子集更不favorable"这个方向性结论看起来站不住，可能是错的**——
750个真实DFT标签，作为真实总体比例的估计，比一个MAE=3.0的零成本代理
模型在5万个pair上的点预测靠谱得多（之前提到的"模型会往均值收缩"这个
担忧，现在看比当初权衡的更重要）。以后把32.4%（n=750下二项分布标准误
~1.7%）当作真实favorable比例的最佳估计，不是23.1%。这不是要推翻flying
dataset基础设施本身的工作（那部分依然正确有用），只是纠正了单靠弱筛选
模型得出的这一条方向性推论。这批数据的`resid_b973c`：均值-5.05，
标准差0.87——跟已确立的~-5 kcal偏移量一致，确认是干净的自洽标签。数据见
