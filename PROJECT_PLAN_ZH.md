...
错了（`candidates_v3` ≠ 真实的 220,860² 对空间），所以 cross AL 会在一个正确的
...
- **原理。** 单体的宇宙。`data/library/aldehydes_clean_v6.csv` = **220,860 个醛** ——
...
- **状态（用户，2026-09-10）：过去的 AL 暂时不采用，cross AL 会重做。** 原因：
  ...
  战役会在正确定义的池（220,860² 空间上的 flying dataset）上跑，等它和更好的标签
  （Tier B / B97-3c）就位。
...
### 2.11 化学空间定义与 "flying dataset" —— ❌ 需要建（用户，2026-09-10）
- **原理。** 一个筛选模型只有它筛的空间有意义才有意义。用户两个修正：
  1. **homo 空间** = 220,860 个 v6 醛的自配对。有些设计上算不出来（§1.2）—— 那是记录
     在案的结果，不是缺口。
  2. **cross 空间** —— 旧 `candidates_v3` 池（~124 万对）是一个**构造错的子集**，不是
     真实空间。真实 cross 空间 = **每个 v6 醛的有向对：220,860² ≈ 4.88 × 10¹⁰**
     （~2.44 × 10¹⁰ 无向；donor/acceptor 角色化学上不同所以有向是诚实计数）。
     `candidates_v3` 应退役为"那个池"。
...
- **状态。** 未建。`CHEMICAL_SPACE_ZH.md`（规范）待写→**已写**。
- **缺。** 冻结的 canonical 醛索引；读取 API；DFT 标签怎么 key 进去的决定；`candidates_v3`
...
- **`candidates_v3` 池是错的。** 真实 cross 空间 = 220,860² 有向醛对；建 flying dataset
...
3. **Flying dataset 规范**（§2.11）—— `CHEMICAL_SPACE_ZH.md` 已写；下一步冻结 canonical
   v6 醛索引、实现 `pair(i, j)` 读取 API、决定 DFT 标签怎么 key、退役 `candidates_v3`。
